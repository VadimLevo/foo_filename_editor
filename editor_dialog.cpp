...
// --- ГЛОБАЛЬНАЯ КОНФИГУРАЦИЯ (СОХРАНЯЕТСЯ ФУБАРОМ НАВСЕГДА) ---
...
// НОВОЕ: GUID'ы для 3-х чекбоксов
...
// НОВОЕ: Переменные чекбоксов (1 - включено, 0 - выключено)
...
wchar_t SafeToUpper(wchar_t c) {
    wchar_t buf[2] = { c, 0 };
    CharUpperW(buf);
    return buf[0];
}

wchar_t SafeToLower(wchar_t c) {
    wchar_t buf[2] = { c, 0 };
    CharLowerW(buf);
    return buf[0];
}

bool SafeIsAlpha(wchar_t c) {
    return IsCharAlphaW(c) != 0;
}
...
void SanitizeFilename(pfc::string8& name) {
    ...
    for (int i = 0; invalid[i]; i++) { wName.Replace(invalid[i], L'_'); }
...
bool IsRomanNumeral(const CString& str) {
    ...
    w.ReleaseBuffer();
    if (w == L"MIX" || w == L"DIV" || w == L"DI" || w == L"MI" || w == L"LI" || w == L"CIV") {
        return false;
    }
...
void ApplyCaseConversion(pfc::string8& name, int mode) {
    ...
    if (mode == 3) {
        ...
        for (int i = 0; i < wName.GetLength(); ++i) {
            wchar_t c = wName[i];
            if (SafeIsAlpha(c)) {
                if (capitalizeNext) { wName.SetAt(i, SafeToUpper(c)); capitalizeNext = false; }
            }
        ...
        if (mode == 1) {
            ...
            if (wName.GetLength() > 0 && SafeIsAlpha(wName[0])) wName.SetAt(0, SafeToUpper(wName[0]));
        }
    }
...
BOOL CEditorDialog::OnInitDialog(CWindow wndFocus, LPARAM lInitParam) {
    ...
    // НОВОЕ: Восстанавливаем состояние чекбоксов
    ...
    if (history.is_empty()) {
        ...
        for (t_size i = 0; i < history.length(); ++i) {
            if (history[i] == '|') {
                if (!current_token.is_empty()) cbPattern.AddString(pfc::stringcvt::string_os_from_utf8(current_token).get_ptr());
                current_token.reset();
            }
            else {
                current_token.add_char(history[i]);
            }
        }
        if (!current_token.is_empty()) cbPattern.AddString(pfc::stringcvt::string_os_from_utf8(current_token).get_ptr());
    }
...
void CEditorDialog::OnDestroy() {
    ...
    // НОВОЕ: Запоминаем чекбоксы перед закрытием
...
void CEditorDialog::SaveHistory() {
    ...
    for (int i = 0; i < cb.GetCount(); ++i) {
        ...
        if (len != CB_ERR) {
            CString text;
            cb.GetLBText(i, text.GetBuffer(len + 1));
            text.ReleaseBuffer();
...
void CEditorDialog::OnRename(UINT uNotifyCode, int nID, CWindow wndCtl) {
    ...
    if (useTags) {
        ...
        pipeline.push_back(std::make_unique<TagRenamerProcessor>(patternUtf8.is_empty() ? "%title%" : patternUtf8.get_ptr()));
    }

    if (useTranslit) {
        pipeline.push_back(std::make_unique<LigatureProcessor>());
        CString repStr;
        GetDlgItemText(IDC_EDT_REPLACE_CHAR, repStr);
        pfc::string8 repUtf8 = pfc::stringcvt::string_utf8_from_os(repStr).get_ptr();
        pipeline.push_back(std::make_unique<TransliteratorProcessor>(repUtf8.get_ptr()));
    }
    ...
    for (t_size i = 0; i < m_items.get_count(); ++i) {
        const char* old_path_raw = m_items[i]->get_path();
        if (strncmp(old_path_raw, "file://", 7) != 0) continue;

        pfc::string8 old_path = old_path_raw + 7;
        ...
        if (MoveFileW(pfc::stringcvt::string_os_from_utf8(old_path).get_ptr(),
            pfc::stringcvt::string_os_from_utf8(new_path).get_ptr())) {
            successCount++;

            old_handles.add_item(m_items[i]);
            playable_location_impl location(new_path.get_ptr(), m_items[i]->get_subsong_index());
            ...
            CString failedName = pfc::stringcvt::string_os_from_utf8(pfc::string_filename_ext(old_path)).get_ptr();
            failedFiles.push_back(failedName);
        }
    }
    ...
    if (successCount > 0) {
        ...
        for (t_size p = 0; p < p_count; ++p) {
            if (pm->playlist_lock_get_filter_mask(p) & playlist_lock::filter_replace) continue;
        ...
        // НОВОЕ: Заставляем foobar2000 прочитать теги из новых файлов прямо сейчас
    ...
    if (failedFiles.empty()) {
        msg.Format(_T("Successfully renamed %d files."), successCount);
        MessageBox(msg, _T("File Name Editor - Success"), MB_OK | MB_ICONINFORMATION);
    }
    else {
        msg.Format(_T("Successfully renamed: %d files.\n\nFailed to rename: %zu files:\n"), successCount, failedFiles.size());
        ...
        msg += _T("\nReason: The files might be playing in foobar2000, locked by another program, or lack write permissions.");
        MessageBox(msg, _T("File Name Editor - Partial Success / Error"), MB_OK | MB_ICONWARNING);
    }
...
void CEditorDialog::UpdatePreview() {
    ...
    for (t_size i = 0; i < m_items.get_count(); ++i) {
        const char* full_path = m_items[i]->get_path();
...
void CEditorDialog::OnPatternSelect(UINT uNotifyCode, int nID, CWindow wndCtl) {
    ...
    if (sel >= 0) {
        ...
        if (len != CB_ERR) {
            CString text;
            cb.GetLBText(sel, text.GetBuffer(len + 1));
            text.ReleaseBuffer();
...
void CEditorDialog::OnTagMenu(UINT uNotifyCode, int nID, CWindow wndCtl) {
    ...
    RECT rect;
    ::GetWindowRect(GetDlgItem(IDC_BTN_TAG_MENU), &rect);
...
void CEditorDialog::OnSavePattern(UINT uNotifyCode, int nID, CWindow wndCtl) {
    CString currentText;
    ::GetWindowText(GetDlgItem(IDC_CMB_TAG_PATTERN), currentText.GetBuffer(256), 256);
...
void CEditorDialog::OnDelPattern(UINT uNotifyCode, int nID, CWindow wndCtl) {
    CComboBox cb(GetDlgItem(IDC_CMB_TAG_PATTERN));
    CString currentText;
    ::GetWindowText(cb.m_hWnd, currentText.GetBuffer(256), 256);
    currentText.ReleaseBuffer();
...
void CEditorDialog::OnPresetSingle(UINT uNotifyCode, int nID, CWindow wndCtl) {
    ...
    if (!currentText.IsEmpty()) {
        ...
        MessageBox(_T("Current pattern saved as default for Single albums."), _T("Saved"), MB_OK | MB_ICONINFORMATION);
    }
}
...
void CEditorDialog::OnPresetVA(UINT uNotifyCode, int nID, CWindow wndCtl) {
    ...
    if (!currentText.IsEmpty()) {
        ...
        MessageBox(_T("Current pattern saved as default for VA albums."), _T("Saved"), MB_OK | MB_ICONINFORMATION);
    }
}
...
void CEditorDialog::OnSetCaseDefault(UINT uNotifyCode, int nID, CWindow wndCtl) {
    ...
    if (sel >= 0) {
        ...
        MessageBox(_T("Current case mode saved as default."), _T("Saved"), MB_OK | MB_ICONINFORMATION);
    }
}